...
function ChampionRotations({ rotation, champs, setDetail }: any) {
  ...
  const handleDtailCilck = (champ: ChampionInfoType) => {
        ...
        <tbody>
          <tr>
            {rotation.map((Id: number) =>
              Object.entries(champs).map(
                (champ: any) =>
                  champ[1].key == Id && (
                    <td className="rotations" key={champ[1].key}>
                      <button onClick={() => handleDtailCilck(champ)}>
                        <img
          ...
          <tr>
            {rotation.map((Id: number) =>
              Object.entries(champs).map(
                (champ: any) =>
                  champ[1].key == Id && (
                    <td key={champ[1].key} className="champ_name">
                      {champ[1].name}
                    </td>
                  )
              )
