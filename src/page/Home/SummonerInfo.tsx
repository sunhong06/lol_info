...
let cancel: Canceler;
 ...
 const getSummonerData = async(summonerName:string) => {
    // 소환사 정보API
    try{  
  const res =  await lolAxios.get(`summoner/v4/summoners/by-name/${summonerName}`,{cancelToken: new axios.CancelToken((c) => (cancel = c))})
      ...
      if(error.response.status == 404){
        setLoding(false)
        return  dispatch({type:"summonerDataReducer/summoner", payload:{error: "소환사의 정보가 없습니다. 다시 검색해주세요."}});
      }
    }
  }
  // 소환사 매치데이터
  ...
  const summonerMatchData = async (Summonerpuuid:number) =>{
    const res2 =  await asiaLolAxios.get(`match/v5/matches/by-puuid/${Summonerpuuid}/ids?start=${startingNum}&count=20&`)
  ...
  return (
    ...
    <main className='summoner_main'>
      ...
      :
          summonerDataSeletor.map((s:summoner)=>  s.name ?
          <div key={s.id}>
